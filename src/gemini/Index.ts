...
async function summaryWithAI(gitDiffMsg: string) {
    ...
    Rules & format:
    - Output only a JSON array of strings. Example: ["<subject>", "- <type>(<scope>): <short desc>", "- <type>(<scope>): <short desc>", "Automaically Generated by Google Gemini [wcm]"]
    - Element 0 — subject: one line, ≤50 characters, terse summary (use imperative tone).
    ...
    - Append a final array element exactly: "Automaically Generated by Google Gemini [wcm]".
    - If the diff shows no changes, return ["No changes", "Automaically Generated by Google Gemini [wcm]"].
    ...
    const data: string[] = JSON.parse(cleanedRes)
    console.log(`[${Tags.AI}] Cleaned Response:`)

    for (let i = 0; i < data.length; i++) {
        const res = data[i];
        console.log(`[${Tags.AI}] ${res}`)
    }
...
async function writeCommitMessage(changes: string[]) {
    ...
    const commands = [
        "git add .",
        "git commit " + changes.map(x => `-m "${x}" `).join(""),
        "git push"
    ]
...
if (gitDiffContent.data) {
    ...
    const changesMessage = await summaryWithAI(gitDiffContent.data)
    const gitAction = await writeCommitMessage(changesMessage)

    if (gitAction == false) {
        console.log(`[${Tags.Git}] Finished with error.`)
    } else {
        console.log(`[${Tags.Git}] Finished with no error.`)
    }
} else {
