...
async function writeCommitMessage(changes: string[], response?: GenerateContentResponse) {
    ...
    const totalTokenRaw = response?.usageMetadata?.totalTokenCount ?? 0 // number - 2381
    const totalTokenFormatted = humanNumber(totalTokenRaw)
    
    changesMessages += `
 ✨ Automatically Generated by Google Gemini (${response?.modelVersion ?? "Unknown Model"}, wasted ${totalTokenFormatted} tokens)`
